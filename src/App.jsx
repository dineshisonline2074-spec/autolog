...
function App() {
  ...
  const user = session
    ? {
        name: session.user.user_metadata?.name || "AutoLog User",
  ...
  if (loading) {
    return <div className="app">Loading AutoLog...</div>;
  }
...
useEffect(() => {
  const loadCloudData = async () => {
    const [vehiclesResult, fuelResult, maintenanceResult, expensesResult, remindersResult] = await Promise.all([
      supabase.from("vehicles").select("*").eq("user_id", user.id).order("created_at", { ascending: true }),
      supabase.from("fuel_entries").select("*").eq("user_id", user.id).order("date", { ascending: false }),
      supabase.from("maintenance_records").select("*").eq("user_id", user.id).order("date", { ascending: false }),
      supabase.from("expenses").select("*").eq("user_id", user.id).order("date", { ascending: false }),
      supabase.from("reminders").select("*").eq("user_id", user.id).order("due_date", { ascending: true }),
    ]);

    if (vehiclesResult.error) console.error("Error loading vehicles:", vehiclesResult.error);
    else setVehicles(vehiclesResult.data || []);
    if (fuelResult.error) console.error("Error loading fuel entries:", fuelResult.error);
    else setFuelEntries((fuelResult.data || []).map((row) => ({ id: row.id, vehicleId: row.vehicle_id, date: row.date, odometer: row.odometer, quantity: row.quantity, pricePerLitre: row.price_per_litre, fullTank: row.full_tank ? "Yes" : "No", amount: row.total_cost ?? Number(row.quantity || 0) * Number(row.price_per_litre || 0), mileage: row.mileage })));
    if (maintenanceResult.error) console.error("Error loading maintenance records:", maintenanceResult.error);
    else setMaintenanceRecords((maintenanceResult.data || []).map((row) => ({ id: row.id, vehicleId: row.vehicle_id, date: row.date, odometer: row.odometer, serviceType: row.service_type, cost: row.cost, notes: row.notes || row.description || "" })));
    if (expensesResult.error) console.error("Error loading expenses:", expensesResult.error);
    else setExpenses((expensesResult.data || []).map((row) => ({ id: row.id, vehicleId: row.vehicle_id, date: row.date, category: row.category, amount: row.amount, notes: row.description || "" })));
    if (remindersResult.error) console.error("Error loading reminders:", remindersResult.error);
    else setReminders((remindersResult.data || []).map((row) => ({ id: row.id, vehicleId: row.vehicle_id, title: row.title, dueDate: row.due_date, type: row.type, dueOdometer: "", notes: "", completed: row.completed })));
  };
  loadCloudData();
}, [user.id]);
  ...
  const deleteVehicle = async (vehicleId) => {
    ...
    if (error) { console.error("Error deleting vehicle:", error); alert(error.message); return; }
    setVehicles((current) => current.filter((vehicle) => vehicle.id !== vehicleId));
    setVehicleToView(null);
    setActivePage("vehicles");
  };

  const saveFuelEntry = async (entry) => {
    const totalCost = Number(entry.quantity || 0) * Number(entry.pricePerLitre || 0);
    const { data, error } = await supabase.from("fuel_entries").insert([{ user_id: user.id, vehicle_id: entry.vehicleId, date: entry.date, odometer: Number(entry.odometer), quantity: Number(entry.quantity), price_per_litre: Number(entry.pricePerLitre), full_tank: entry.fullTank === "Yes" || entry.fullTank === true, total_cost: totalCost, mileage: entry.mileage ? Number(entry.mileage) : null }]).select().single();
    if (error) { console.error("Error saving fuel entry:", error); alert(error.message); return; }
    setFuelEntries((current) => [{ id: data.id, vehicleId: data.vehicle_id, date: data.date, odometer: data.odometer, quantity: data.quantity, pricePerLitre: data.price_per_litre, fullTank: data.full_tank ? "Yes" : "No", amount: data.total_cost, mileage: data.mileage }, ...current]);
    setIsFuelFormOpen(false);
  };

  const deleteFuelEntry = async (entryId) => {
    const { error } = await supabase.from("fuel_entries").delete().eq("id", entryId).eq("user_id", user.id);
    if (error) { console.error("Error deleting fuel entry:", error); alert(error.message); return; }
    setFuelEntries((current) => current.filter((entry) => entry.id !== entryId));
  };

  const saveMaintenanceRecord = async (record) => {
    const { data, error } = await supabase.from("maintenance_records").insert([{ user_id: user.id, vehicle_id: record.vehicleId, date: record.date, odometer: Number(record.odometer), service_type: record.serviceType, description: record.notes || null, cost: Number(record.cost), notes: record.notes || null }]).select().single();
    if (error) { console.error("Error saving maintenance record:", error); alert(error.message); return; }
    setMaintenanceRecords((current) => [{ id: data.id, vehicleId: data.vehicle_id, date: data.date, odometer: data.odometer, serviceType: data.service_type, cost: data.cost, notes: data.notes || data.description || "" }, ...current]);
    setIsMaintenanceFormOpen(false);
  };

  const deleteMaintenanceRecord = async (recordId) => {
    const { error } = await supabase.from("maintenance_records").delete().eq("id", recordId).eq("user_id", user.id);
    if (error) { console.error("Error deleting maintenance record:", error); alert(error.message); return; }
    setMaintenanceRecords((current) => current.filter((record) => record.id !== recordId));
  };

  const saveExpense = async (expense) => {
    const { data, error } = await supabase.from("expenses").insert([{ user_id: user.id, vehicle_id: expense.vehicleId, date: expense.date, category: expense.category, description: expense.notes || null, amount: Number(expense.amount) }]).select().single();
    if (error) { console.error("Error saving expense:", error); alert(error.message); return; }
    setExpenses((current) => [{ id: data.id, vehicleId: data.vehicle_id, date: data.date, category: data.category, amount: data.amount, notes: data.description || "" }, ...current]);
    setIsExpenseFormOpen(false);
  };

  const deleteExpense = async (expenseId) => {
    const { error } = await supabase.from("expenses").delete().eq("id", expenseId).eq("user_id", user.id);
    if (error) { console.error("Error deleting expense:", error); alert(error.message); return; }
    setExpenses((current) => current.filter((expense) => expense.id !== expenseId));
  };

  const saveReminder = async (reminder) => {
    const { data, error } = await supabase.from("reminders").insert([{ user_id: user.id, vehicle_id: reminder.vehicleId, title: reminder.title, due_date: reminder.dueDate, type: reminder.type, completed: false }]).select().single();
    if (error) { console.error("Error saving reminder:", error); alert(error.message); return; }
    setReminders((current) => [{ id: data.id, vehicleId: data.vehicle_id, title: data.title, dueDate: data.due_date, type: data.type, completed: data.completed, dueOdometer: "", notes: "" }, ...current]);
    setIsReminderFormOpen(false);
  };

  const deleteReminder = async (reminderId) => {
    const { error } = await supabase.from("reminders").delete().eq("id", reminderId).eq("user_id", user.id);
    if (error) { console.error("Error deleting reminder:", error); alert(error.message); return; }
  ...
  return (
    <>
      <section className="welcome">
          <div>
            <p className="muted">Welcome back, Dinesh 👋</p>
